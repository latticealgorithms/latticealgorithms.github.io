...
#+LATEX: \noindent
Let $\qq(n)$ and $\pfactor(n)$ be functions.\\
...
Input: $\pB$ for lattice $\pL$ of dimension $n$, target vector $\vt = \pB\cdot \vs + \vD$, $\|\vD\| \leq \pfactor(\dL) \lambda_1(\pL)$
  ...
  2. For $i = 1, \ldots , m(n)$:
     1. Compute $\va_i = \dv_i \cdot \pB \bmod \qq(n)$.
     2. Compute $b_i = \dv_i \cdot \vt \bmod \qq(n)$.
  3. Call the $\LWE$ oracle with $(\va_1,b_1), \ldots, (\va_{m(n)}, b_{m(n)})$ and get part of the secret coefficient vector, namely $\vs \bmod \qq(n)$.
  4. Repeat this for different values of $\qq(n)$.
...
** The quantum reduction from \(\varphi_0(\dL)\)-$\SIVP$ to $\LWE_{n,\qq(n),\alpha(n)}$.
