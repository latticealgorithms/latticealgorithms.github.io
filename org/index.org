...
| $\GapSVL$                                      | As a function of the dimension $n$, period $q(n)$ and finite group rank $r(n)$ this problem is easy for randomly selected lattices.  With high probability, $\delta \sqrt{n} \det(L)^{1/n} \leq \lambda_1(L) \leq \sqrt{n}\det(L)^{1/n}$ for a constant $\delta$.
